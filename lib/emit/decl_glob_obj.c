/** @file decl_glob_obj.c */


#include "./decl_glob_obj.h"
#include "aclspv.h"
...
#define	INFO			(((INTERFACES + ARG_IDX)))
	...
	INFO->m_unified.m_var_elm_type_id	= aclutil_mk_constant_struct_id(1, CTX);
	INFO->m_unified.m_var_type_id		= aclutil_mk_constant_ptr_storage_id(1, CTX);
	...
	switch(INFO->m_unified.m_storage_class) {
		...
		case SpvStorageClassWorkgroup:
...

			{
				...
				const aclspv_id_t VAL_ELM	= aclutil_mk_constant_val_id(ELM_ARR, CTX);
				const aclspv_id_t VAL_2		= aclutil_mk_constant_val_id(2, CTX);

				const aclspv_id_t VAL_3		= aclutil_mk_constant_val_id(3, CTX);
				...
				ae2f_expected_if(SPECID_WORK != 0xFFFFFFFF) {
					const aclspv_id_t _SPECIDSPIRV 
						= aclutil_mk_constant_spec_id(SPECID_WORK, NUM_ARR, CTX);

					const aclspv_id_t SPECIDSPIRV
						= _SPECIDSPIRV ? _SPECIDSPIRV : 
						aclutil_mk_constant_spec64_id(
								SPECID_WORK
								, NUM_ARR
								, CTX
								);

					ae2f_expected_but_else(SPECIDSPIRV) {
						CTX->m_err = ACLSPV_COMPILE_MET_INVAL;
						return CXChildVisit_Break; 
					}

					INFO->m_work.m_arr_count_id = SPECIDSPIRV;
				} else {
					INFO->m_work.m_arr_count_id = aclutil_mk_constant_val_id(NUM_ARR, CTX);
				}
			...
			ae2f_unexpected_but_if(0) {
				...
				/** TODO: those are hard-coded. Should be dynamicly loaded. */
				case SpvStorageClassInput:
				case SpvStorageClassOutput:
				{
					const aclspv_id_t I32 = aclid_mk_default_id(ID_DEFAULT_I32, CTX);
					const aclspv_id_t I32V4 = aclutil_mk_cxtp_vec(
							ID_DEFAULT_I32
							, 4, CTX);

					ae2f_expected_but_else(I32V4 && I32) {
						return CXChildVisit_Break;
					}

					/** INFO->m_unified.m_storage_class */

					INFO->m_unified.m_var_type_id = aclutil_mk_cxtp_ptr(
							I32V4
							, INFO->m_unified.m_storage_class
							, CTX
							);
...
#define		THIS_ENTP	((util_entp_t* ae2f_restrict)CTX->m_fnlist.m_entp.m_p)[CTX->m_tmp.m_w0]
			...
			INFO->m_unified.m_var_elm_type_id	= aclutil_mk_constant_vec32_id(4, CTX);
...
ACLSPV_ABI_IMPL enum CXChildVisitResult aclemit_decl_glob_obj(CXCursor h_cur, CXCursor h_cur_parent, CXClientData h_ctx) {
#define CTX	((x_aclspv_ctx* ae2f_restrict)h_ctx)
	...
	if(util_is_kernel(h_cur_parent)) {
		...
		ae2f_expected_but_else(CTX->m_tmp.m_v0.m_p) {
			CTX->m_err = ACLSPV_COMPILE_ALLOC_FAILED_DECLGLOBOBJ;
			return CXChildVisit_Break;
		}
		...
		if(BUFF[0]) {
			const aclspv_id_t PSH_PTR_ID 	= aclutil_mk_constant_ptr_psh_id((aclspv_wrd_t)BUFF[0], CTX);
...
#define EMIT_POS	CTX->m_count.m_name
			...
			ae2f_expected_but_else((EMIT_POS = emit_opcode(
							&CTX->m_section.m_name
							, EMIT_POS, SpvOpName, 0))) 
				goto LBL_DBG_STR_DISPOSE_FAIL;
...
LBL_DBG_STR_DISPOSE_FAIL:
			...
			((util_entp_t* ae2f_restrict)CTX->m_fnlist.m_entp.m_p)[CTX->m_tmp.m_w0].m_push_ids.m_push_struct
				= aclutil_mk_constant_struct_id((aclspv_wrd_t)BUFF[0], CTX);
